...
exportTeam=function exportTeam(team){var gen=arguments.length>1&&arguments[1]!==undefined?arguments[1]:this.gen;var hidestats=arguments.length>2&&arguments[2]!==undefined?arguments[2]:false;
if(!team)return'';
if(typeof team==='string'){
if(team.indexOf('\n')>=0)return team;
team=this.unpackTeam(team);
}
var text='';for(var _i62=0,_team2=
team;_i62<_team2.length;_i62++){var curSet=_team2[_i62];
...
if(curSet.ivs){
var defaultIvs=true;
var hpType='';for(var _i64=0,_curSet$moves2=
curSet.moves;_i64<_curSet$moves2.length;_i64++){var move=_curSet$moves2[_i64];
...
var _stat9=void 0;
for(_stat9 in BattleStatNames){var _Dex$types$get$HPivs;
if((curSet.ivs[_stat9]===undefined?31:curSet.ivs[_stat9])!==(((_Dex$types$get$HPivs=Dex.types.get(hpType).HPivs)==null?void 0:_Dex$types$get$HPivs[_stat9])||31)){
defaultIvs=false;
break;
}
}
...
var _stat10=void 0;
for(_stat10 in BattleStatNames){
if(curSet.ivs[_stat10]!==31&&curSet.ivs[_stat10]!==undefined){
defaultIvs=false;
break;
}
}
...
if(!defaultIvs){
var _stat11=void 0;
for(_stat11 in BattleStatNames){
if(typeof curSet.ivs[_stat11]==='undefined'||isNaN(curSet.ivs[_stat11])||curSet.ivs[_stat11]===31)continue;
if(first){
text+='IVs: ';
first=false;
}else{
...
text+=''+curSet.ivs[_stat11]+' '+BattleStatNames[_stat11];
}
}
}
if(!first){
...
if(curSet.moves){for(var _i66=0,_curSet$moves4=
curSet.moves;_i66<_curSet$moves4.length;_i66++){var _move2=_curSet$moves4[_i66];
if(_move2.substr(0,13)==='Hidden Power '){
_move2=_move2.substr(0,13)+'['+_move2.substr(13)+']';
}
if(_move2){
text+='- '+_move2+"  \n";
}
...
break;
default:for(var _i68=0,_this$sides6=
this.sides;_i68<_this$sides6.length;_i68++){var side=_this$sides6[_i68];side.active=[null];}
break;
}
...
case'showteam':{
var team=this.unpackTeam(args[2]);
if(!team.length)return;
var _side6=this.getSide(args[1]);
_side6.clearPokemon();for(var _i70=0;_i70<
...
if(set.ability)_pokemon8.rememberAbility(set.ability);for(var _i72=0,_set$moves2=
...
var _move3=Dex.moves.get(args[2]);
if(this.checkActive(_poke44))return;
var poke2=this.getPokemon(args[3]);
this.scene.beforeMove(_poke44);
this.useMove(_poke44,_move3,poke2,kwArgs);
this.animateMove(_poke44,_move3,poke2,kwArgs);
this.log(args,kwArgs);
...
var _move4=Dex.moves.get(args[3]);
this.cantUseMove(_poke45,effect,_move4,kwArgs);
this.log(args,kwArgs);
break;
}
case'gen':{
...
stack.length;_i74++){var line=stack[_i74];
if(/\brun\b/.test(line)){
break;
}
this.log(['error',line]);
